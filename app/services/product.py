...
class ProductService:
    ...
    async def create_product(self, product_data: ProductCreate) -> ProductRead:
        ...
        new_product = await self.product_repo.add_one(product_data.model_dump())
    ...
    async def get_products(self, skip: int = 0, limit: int = 10) -> ProductList:
        ...
        page = (skip // limit) + 1
        products = await self.product_repo.find_all(skip=skip, limit=limit)
        return ProductList(
    ...
    async def get_product(self, product_id: int) -> ProductRead:
        product = await self.product_repo.find_one(id=product_id)
        if not product:
